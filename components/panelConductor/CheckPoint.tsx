...
export default function CheckPoint({
    ...
    return (<div key={`ruta_segmento_${index}`} className="flex mt-1 items-center overflow-hidden">
        <BsFillGeoAltFill size={`${esTitulo ? '1.5rem' : '1.1rem'}`} 
            className={esTitulo ? 'w-8' : 'w-4'} />
        <span className={`${esTitulo ? 'text-2xl w-56' : 'flex flex-col text-xs w-36 h-12 justify-center'} ml-2`}>{destino && destino.direccion
        ...
        {isLast && <button
            ...
            onClick={() => {
                const glosaDestino = `${destino && destino.direccion && destino.direccion.latitud},${destino && destino.direccion && destino.direccion.longitud}`;
