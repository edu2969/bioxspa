...
exports.default = AsignacionPanel;
...
function AsignacionPanel({ session }) {
    const [pedidos, setPedidos] = (0, react_2.useState)([]);
    const [choferes, setChoferes] = (0, react_2.useState)([]);
    const [enTransito, setEnTransito] = (0, react_2.useState)([]);
    const [showConfirmModal, setShowConfirmModal] = (0, react_2.useState)(false);
    const [showReasignacionModal, setShowReasignacionModal] = (0, react_2.useState)(false);
    const [selectedChofer, setSelectedChofer] = (0, react_2.useState)(null);
    const [selectedPedido, setSelectedPedido] = (0, react_2.useState)(null);
    const [selectedVenta, setSelectedVenta] = (0, react_2.useState)(null);
    const [loading, setLoading] = (0, react_2.useState)(false);
    const [loadingPanel, setLoadingPanel] = (0, react_2.useState)(true);
    ...
    const fetchPedidos = (0, react_1.useCallback)(async () => {
        try {
            ...
            setEnTransito(data.flotaEnTransito.map((ruta) => {
                return Object.assign(Object.assign({}, ruta), { resumenCarga: getResumenCarga(ruta.cargaItemIds) });
            }));
    ...
    return (<main className="mt-10 h-screen overflow-hidden">
            <div className={`grid grid-cols-12 h-[calc(100vh-40px)] gap-4 p-4 overflow-hidden ${loadingPanel ? "opacity-50" : ""}`}>
                <div className="col-span-7 flex flex-col md:flex-row gap-4">
                    <div className="relative w-1/2 border rounded-lg p-4 bg-white shadow-md h-[calc(100vh-80px)] overflow-y-auto" onDragOver={(e) => {
            e.preventDefault();
            ...
            if (selectedVenta) {
                setShowReasignacionModal(true);
                        ...
                        <div className="flex items-center mb-4">
                            ...
                            <link_1.default href="/modulos/pedidos/nuevo" className="relative ml-auto -mt-2">
                                <button className="flex items-center bg-blue-500 text-white h-12 w-12 rounded hover:bg-blue-600 transition-colors font-semibold">
                                    <fa_1.FaCartPlus size={38} className="pl-0.5 ml-0.5"/>
                                </button>
                            </link_1.default>
                        </div>
                        {pedidos.length === 0 ? (<div className="flex items-center justify-center" style={{ height: "calc(100vh - 200px)" }}>
                                <p className="text-gray-500 text-lg font-semibold">SIN PEDIDOS</p>
                            </div>) : (pedidos.map((pedido, index) => (<div key={`pedido_${index}`} className="p-2 border rounded-lg mb-2 bg-gray-100 cursor-pointer flex items-start relative" draggable onDragStart={() => setSelectedPedido(pedido._id)}>
                                    <div>
                                        <p className="text-md font-bold uppercase w-full">{pedido.clienteNombre}</p>
                                        <p className="text-xs text-gray-500 ml-2">{(0, dayjs_1.default)(pedido.createdAt).fromNow()}</p>
                                        <ul className="list-disc ml-4 mt-2">
                                            {pedido.items.map((item, index2) => (<li key={`item_${index2}`}>{item.cantidad}x {item.nombre}</li>))}
                                        </ul>
                                    </div>
                                    <div className="absolute top-2 right-2 text-gray-500">
                                        <md_1.MdDragIndicator size="1.5rem"/>
                                    </div>
                                </div>)))}
                    </div>
                    ...
                    <div className="relative w-1/2 border rounded-lg p-4 bg-white shadow-md h-[calc(100vh-80px)] overflow-y-auto pt-14">
                        ...
                        {choferes.map((chofer, index) => (<div key={`en_espera_${index}`} className="p-2 border rounded-lg mb-2 bg-gray-100" data-id={`choferId_${chofer._id}`} onDragOver={(e) => {
                e.preventDefault();
                ...
                if (selectedPedido) {
                    ...
                    setShowConfirmModal(true);
                                ...
                                <div className="font-bold uppercase flex">
                                    <go_1.GoCopilot size="1.5rem"/><span className="ml-2">{chofer.nombre}</span>
                                </div>
                                {chofer.pedidos.length ? chofer.pedidos.map((pedido, indexPedido) => {
                var _a;
                return <div className="bg-gray-200 rounded shadow-md p-1 mb-2" onDragStart={() => {
                        setSelectedChofer(chofer._id);
                        ...
                        console.log("Venta seleccionada:", pedido.items[0].ventaId);
                                    ...
                                    <ul className="list-disc ml-4">
                                        {(_a = pedido.items) === null || _a === void 0 ? void 0 : _a.map((item, indexItem) => <li key={`item_en_espera_${indexItem}`}>{item.cantidad}x {item.nombre}</li>)}
                                    </ul>
                                </div>;
                                    ...
                                    <div className="relative -top-8 left-64 bg-gray-400 text-white text-xs font-bold px-2 py-1 rounded-tr-md rounded-bl-md flex items-center">
                                        <ri_1.RiZzzFill size="1rem" className="mr-1"/>
                                        <p>SIN PEDIDOS</p>
                                    </div>
                                </div>}
                            </div>))}
                ...
                <div className="relative col-span-5 border rounded-lg p-4 bg-white shadow-md h-[calc(100vh-80px)] overflow-y-auto overflow-x-hidden pt-12">
                    ...
                    {enTransito.length === 0 ? (<div className="flex items-center justify-center" style={{ height: "calc(100vh - 200px)" }}>
                            <p className="text-gray-500 text-lg font-semibold">NADIE EN RUTA</p>
                        </div>) : (enTransito.map((ruta, index) => {
            var _a;
            return (<div key={`ruta_${index}`} data-id={ruta._id} className="relative w-full border rounded-lg px-4 bg-gray-100 shadow-md mb-4 h-64 pt-4" onDragOver={(e) => {
                    e.preventDefault();
                    ...
                    e.currentTarget.style.backgroundColor = "green";
                                ...
                                <image_1.default className="absolute top-4 left-0 ml-8" src="/ui/camion.png" alt={`camion_atras_${index}`} width={247} height={191} style={{ width: '247px', height: '191px' }} priority/>
                                <div className="absolute top-0 left-0 ml-10 mt-2 w-full h-fit">
                                    {ruta.estado != constants_1.TIPO_ESTADO_RUTA_DESPACHO.regreso && Array.from({ length: ruta.cargaItemIds.length }, (_, i) => ruta.cargaItemIds.length - i - 1).map(index => {
                    ...
                    return (<image_1.default key={index} src={`/ui/tanque_biox${color.length > 1 ? "_" + color : ""}.png`} alt={`tank_${index}`} width={14 * 2} height={78 * 2} className={`absolute ${ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga ? "" : "opacity-20"}`} style={calculateTubePosition(index)} priority={false}/>);
                })}
                                </div>
                                <image_1.default className="absolute top-4 left-0 ml-8" src="/ui/camion_front.png" alt="camion" width={247} height={191} style={{ width: '247px', height: '191px' }}/>
                                <div className="absolute ml-16 mt-6" style={{ transform: "translate(60px, 34px) skew(0deg, -20deg)" }}>
                                    <div className="ml-4 text-slate-800">
                                        ...
                                        <div className={`flex items-center mb-2 ${ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.en_ruta ? 'text-green-700' : ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga ? 'text-orange-500' : 'text-gray-500'}`}>
                                            {ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.en_ruta && <span className="inline-block w-3 h-3 rounded-full bg-green-500 mr-2"></span>}
                                            {(ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga
                    || ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga_confirmada) && <span className="inline-block w-3 h-3 rounded-full bg-orange-500 mr-2"></span>}
                                            {ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.regreso && <span className="inline-block w-3 h-3 rounded-full bg-blue-500 mr-2"></span>}
                                            <span className="text-xs font-semibold">{ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.en_ruta ? 'EN RUTA'
                    : ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga || ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga_confirmada ? 'DESCARGA'
                        : ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.regreso ? 'REGRESO' : 'OTRO'}</span>
                                        </div>
                                    </div>
                                </div>
                                {(ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga
                    || ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga_confirmada) && <div className="absolute top-6 left-8 ml-2 mt-2 w-full">
                                    {Array.from({ length: ruta.cargaItemIds.length }, (_, i) => ruta.cargaItemIds.length - i - 1).map(index => {
                        const elem = ruta.cargaItemIds[index].subcategoriaCatalogoId.categoriaCatalogoId.elemento;
                        const elementos = ["o2", "co2", "n2o", "ar", "he", "aligal", "aire alphagaz", "n2 (liquido)", "n2", "atal", "arcal", "c2h2",];
                        const colores = ["verde", "azul", "rojo", "amarillo", "azul", "rojo", "amarillo", "verde", "rojo", "rojo", "azul", "azul", "rojo"];
                        const color = colores[elementos.indexOf(elem.toLowerCase())] || "";
                        return (<image_1.default key={index} src={`/ui/tanque_biox${color.length > 1 ? "_" + color : ""}.png`} alt={`tank_${index}`} width={14 * 3} height={78 * 3} className={`absolute ${ruta.estado == constants_1.TIPO_ESTADO_RUTA_DESPACHO.descarga_confirmada ? "" : "opacity-40"}`} style={calculateUploadTubePosition(index)} priority={false}/>);
                    })}
                                </div>}
                                <div className="absolute bottom-4 flex items-center ml-4">
                                    <bs_1.BsGeoAltFill size="1.25rem" className="text-gray-700 dark:text-gray-300 ml-2"/>
                                ...
                                <div className="ml-72">
                                    ...
                                    <div className="flex flex-wrap">
                                        {ruta.estado != constants_1.TIPO_ESTADO_RUTA_DESPACHO.regreso ? (_a = ruta.resumenCarga) === null || _a === void 0 ? void 0 : _a.map((item, idx) => (<div key={idx} className="mb-1 border rounded border-gray-400 mr-2 orbitron px-1">
                                            ...
                                            </div>)) : (<div className="text-gray-500 text-xs">REGRESANDO</div>)}
                                    </div>
                                </div>
                            </div>);
        }))}
            ...
            <ConfirmModal_1.ConfirmModal show={showConfirmModal} loading={loading} title="Confirmar Asignación" confirmationQuestion={`¿Estás seguro de asignar este pedido a ${nombreChofer(selectedChofer)}?`} onClose={() => {
     ...
     onConfirm={() => {
            ...
            const assignPedido = async () => {
                try {
                    ...
                    socket_client_1.socket.emit("update-pedidos", { room: "room-pedidos", userId: selectedChofer });
                    react_toastify_1.toast.success("Pedido asignado con éxito");
                    setLoadingPanel(true);
                    fetchPedidos();
                }
                catch (error) {
                    console.error("Error al asignar el pedido:", error);
                    react_toastify_1.toast.error(error.message || "Error al asignar el pedido");
                }
                finally {
            ...
            <ConfirmModal_1.ConfirmModal show={showReasignacionModal} title="Confirmar operación crítica" confirmationQuestion={`¿Estás seguro de deshacer la asignación de este pedido?`} loading={loading} onClose={() => {
     ...
     onConfirm={() => {
            ...
            const deshacerAsignarPedido = async () => {
                try {
                    ...
                    socket_client_1.socket.emit("update-pedidos", { room: "room-pedidos", userId: selectedChofer });
                    react_toastify_1.toast.success("Pedido listo para asignar");
                    fetchPedidos();
                }
                catch (error) {
                    react_toastify_1.toast.error(error.message || "Error al deshacer la asignación del pedido");
                }
                finally {
