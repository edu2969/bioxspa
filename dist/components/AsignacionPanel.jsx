...
function AsignacionPanel() {
    ...
    return (<main className="mt-4 h-screen overflow-hidden">
            ...
            <div className="grid grid-cols-12 h-[calc(100vh-80px)] gap-4 p-4 overflow-hidden">
                ...
                var _a;
                return <div key={`pedidos_chofer_${indexPedido}`}>
                ...
                <div className="col-span-5 border rounded-lg p-4 bg-white shadow-md h-[calc(100vh-80px)] overflow-y-auto">
                    ...
                    {enTransito.length === 0 ? (<div className="flex items-center justify-center" style={{ height: "calc(100vh - 200px)" }}>
                        ...
                        </div>) : (enTransito.map((truck, index) => (<div key={`truck_${index}`} data-id={truck._id} className="relative w-full border rounded-lg px-4 bg-gray-100 shadow-md mb-4 h-64 pt-4" onDragOver={(e) => {
                e.preventDefault();
                ...
                e.currentTarget.style.backgroundColor = "green";
                                ...
                                <image_1.default className="absolute top-4 left-0 ml-12" src="/ui/camion.png" alt={`camion_atras_${index}`} width={247} height={191} style={{ width: '247px', height: '191px' }} priority/>
                                <div className="absolute top-0 left-0 ml-12 mt-2 w-full h-fit">
                                    {Array.from({ length: 6 }).map((_, layerIndex) => (<div key={`${layerIndex}_${truck.patente}`} className="absolute flex" style={calculateTubePosition(layerIndex, 0)}>
                                            {Array.from({ length: 6 }).map((_, index) => (<image_1.default key={index} src={`/ui/tanque_biox${(index + layerIndex * 6 > 40) ? '_verde' : (index + layerIndex * 6 > 20) ? '_azul' : ''}.png`} alt={`tank_${layerIndex * 6 + index}`} width={14} height={78} className='relative' style={calculateTubePosition(layerIndex, index)} priority={false}/>))}
                                        </div>))}
                                </div>
                                <image_1.default className="absolute top-4 left-0 ml-12" src="/ui/camion_front.png" alt="camion" width={247} height={191} style={{ width: '247px', height: '191px' }}/>
            ...
            <ConfirmModal_1.ConfirmModal show={showConfirmModal} title="Confirmar Asignación" confirmationQuestion={`¿Estás seguro de asignar este pedido a ${nombreChofer(selectedChofer)}?`} onClose={() => {
