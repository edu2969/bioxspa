...
function MessagesPanel({ visible, onClick }) {
    ...
    return (<div style={{
            ...
            <div className="flex p-4">
                <button onClick={() => onClick()} className="text-4xl cursor-pointer -mt-2">
                    &times;
                </button>
            ...
            return (<li key={topic.topicId} className="p-2 rounded-lg shadow mb-2 flex items-start cursor-pointer hover:bg-gray-100">
                            ...
                            <div className="flex items-center ml-2">
                                <image_1.default width={8} height={8} src={`/profiles/${getUserById(topic.userIdOwner).avatar}`} alt={getUserById(topic.userIdOwner).name} className="w-8 h-8 rounded-full border-2 border-white -ml-4"/>
                                {topic.userIdsInvited.map(userId => {
                    const invitedUser = getUserById(userId);
                    return (<image_1.default width={8} height={8} key={userId} src={`/profiles/${invitedUser.avatar}`} alt={invitedUser.name} className="w-8 h-8 rounded-full border-2 border-white -ml-4"/>);
                })}
                            </div>
                        </li>);
        })}
