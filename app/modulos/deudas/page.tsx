...
export default function Debtors() {
    ...
    const riskText = (risk: number) => {
        if (risk <= 30) return "BIEN";
        if (risk <= 60) return "RIESGOSO";
        return "QUIEBRA";
    };
                ...
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg p-2">
                    ...
                    <div>
                        {debtors && debtors.map(debtor => (
                            <div key={debtor.id} className="flex bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-600">
                                ...
                                <div className="w-1/12 pt-4">
                                    <CircularProgressbar className="h-14" strokeWidth={16} value={debtor.risk} text={`${debtor.risk}%`} />
                                ...
                                <div className="w-2/12 px-2 py-7 flex text-center justify-center">
                                    <Link href={`/modulos/deudas/${debtor.id}`} className="hover:text-blue-400 shadow-xl rounded-md w-20 h-14 mr-2 pt-2">
                                        <MdOutlineVisibility size="1.2rem" className="mx-auto" /><span className="text-xs">VER DEUDA</span>
                                    </Link>
                                </div>
                            </div>
                        ))}
