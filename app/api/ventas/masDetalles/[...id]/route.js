import { NextResponse } from "next/server";
import { getServerSession } from "next-auth/next";
import { authOptions } from "@/app/utils/authOptions";
import { connectMongoDB } from "@/lib/mongodb";
import Venta from "@/models/venta";
import Cliente from "@/models/cliente";
import User from "@/models/user";
import Sucursal from "@/models/sucursal";
import Dependencia from "@/models/dependencia";
import Direccion from "@/models/direccion";
import DocumentoTributario from "@/models/documentoTributario";
import DetalleVenta from "@/models/detalleVenta";
import SubcategoriaCatalogo from "@/models/subcategoriaCatalogo";
import CategoriaCatalogo from "@/models/categoriaCatalogo";
import ItemCatalogo from "@/models/itemCatalogo";
import RutaDespacho from "@/models/rutaDespacho";
import Vehiculo from "@/models/vehiculo";
import mongoose from "mongoose";

export async function GET(request, { params }) {
    try {
        console.log("Connecting to MongoDB...");
        await connectMongoDB();
        console.log("MongoDB connected.");

        // Register models if not already registered
        if (!mongoose.models.User) {
            mongoose.model("User", User.schema);
        }
        if (!mongoose.models.Sucursal) {
            mongoose.model("Sucursal", Sucursal.schema);
        }
        if (!mongoose.models.Dependencia) {
            mongoose.model("Dependencia", Dependencia.schema);
        }
        if (!mongoose.models.Direccion) {
            mongoose.model("Direccion", Direccion.schema);
        }
        if (!mongoose.models.DocumentoTributario) {
            mongoose.model("DocumentoTributario", DocumentoTributario.schema);
        }
        if (!mongoose.models.Venta) {
            mongoose.model("Venta", Venta.schema);
        }
        if (!mongoose.models.DetalleVenta) {
            mongoose.model("DetalleVenta", DetalleVenta.schema);
        }
        if (!mongoose.models.SubcategoriaCatalogo) {
            mongoose.model("SubcategoriaCatalogo", SubcategoriaCatalogo.schema);
        }
        if (!mongoose.models.CategoriaCatalogo) {
            mongoose.model("CategoriaCatalogo", CategoriaCatalogo.schema);
        }
        if (!mongoose.models.ItemCatalogo) {
            mongoose.model("ItemCatalogo", ItemCatalogo.schema);
        }
        if (!mongoose.models.RutaDespacho) {
            mongoose.model("RutaDespacho", RutaDespacho.schema);
        }
        if (!mongoose.models.Vehiculo) {
            mongoose.model("Vehiculo", Vehiculo.schema);
        }
        if (!mongoose.models.Cliente) {
            mongoose.model("Cliente", Cliente.schema);
        }

        console.log("Fetching server session...");
        const session = await getServerSession(authOptions);

        if (!session || !session.user || !session.user.id) {
            console.warn("Unauthorized access attempt.");
            return NextResponse.json({ ok: false, error: "Unauthorized" }, { status: 401 });
        }

        // Await params before accessing its properties
        const resolvedParams = await params;
        const ventaId = resolvedParams.id[0];
        console.log(`Fetching venta details for ID: ${ventaId}`);

        if (!mongoose.Types.ObjectId.isValid(ventaId)) {
            return NextResponse.json({ ok: false, error: "Invalid venta ID" }, { status: 400 });
        }

        const venta = await Venta.findById(ventaId)
            .populate({
                path: "clienteId",
                model: "Cliente",
                select: "nombre giro email telefono rut"
            })
            .populate({
                path: "vendedorId",
                model: "User",
                select: "name email"
            })
            .populate({
                path: "sucursalId", 
                model: "Sucursal",
                select: "nombre direccionId",
                populate: {
                    path: "direccionId",
                    model: "Direccion",
                    select: "nombre direccion comuna ciudad region"
                }
            })
            .populate({
                path: "dependenciaId",
                model: "Dependencia", 
                select: "nombre direccionId",
                populate: {
                    path: "direccionId",
                    model: "Direccion",
                    select: "nombre direccion comuna ciudad region"
                }
            })
            .populate({
                path: "documentoTributarioId",
                model: "DocumentoTributario"
            })
            .populate({
                path: "comentariosCobro.userId",
                model: "User",
                select: "name email"
            })
            .lean();

        if (!venta) {
            console.warn(`No venta found for ID: ${ventaId}`);
            return NextResponse.json({ ok: false, error: "Venta not found" }, { status: 404 });
        }

        console.log("Fetching detalles de venta...");
        // Buscar detalles de venta con poblaciones completas
        const detallesVenta = await DetalleVenta.find({ ventaId: ventaId })
            .populate({
                path: "subcategoriaCatalogoId",
                model: "SubcategoriaCatalogo",
                populate: {
                    path: "categoriaCatalogoId",
                    model: "CategoriaCatalogo"
                }
            })
            .populate({
                path: "itemCatalogoIds",
                model: "ItemCatalogo"
            })
            .lean();

        console.log("Fetching ruta de despacho...");
        // Buscar ruta de despacho relacionada con esta venta
        const rutaDespacho = await RutaDespacho.findOne({ 
            ventaIds: { $in: [ventaId] }
        })
            .populate({
                path: "vehiculoId",
                model: "Vehiculo"
            })
            .populate({
                path: "choferId",
                model: "User",
                select: "name email"
            })
            .populate({
                path: "dependenciaId",
                model: "Dependencia",
                select: "nombre direccionId",
                populate: {
                    path: "direccionId",
                    model: "Direccion"
                }
            })
            .populate({
                path: "ruta.direccionDestinoId",
                model: "Direccion"
            })
            .populate({
                path: "ventaIds",
                model: "Venta",
                select: "clienteId direccionDespachoId"
            })
            .populate({
                path: "cargaItemIds",
                model: "ItemCatalogo",
                populate: {
                    path: "subcategoriaCatalogoId",
                    model: "SubcategoriaCatalogo",
                    populate: {
                        path: "categoriaCatalogoId",
                        model: "CategoriaCatalogo"
                    }
                }
            })
            .lean();

        const ventaDetails = {
            _id: venta._id,
            fecha: venta.fecha,
            estado: venta.estado,
            porCobrar: venta.porCobrar,
            valorNeto: venta.valorNeto,
            valorIva: venta.valorIva,
            valorTotal: venta.valorTotal,
            vendedor: {
                _id: venta.vendedorId?._id,
                nombre: venta.vendedorId?.name,
                email: venta.vendedorId?.email
            },
            cliente: {
                _id: venta.clienteId?._id,
                nombre: venta.clienteId?.nombre,
                giro: venta.clienteId?.giro,
                email: venta.clienteId?.email,
                telefono: venta.clienteId?.telefono,
                rut: venta.clienteId?.rut
            },
            sucursal: {
                _id: venta.sucursalId?._id,
                nombre: venta.sucursalId?.nombre,
                direccion: venta.sucursalId?.direccionId ? {
                    _id: venta.sucursalId.direccionId._id,
                    nombre: venta.sucursalId.direccionId.nombre,
                    direccion: venta.sucursalId.direccionId.direccion,
                    comuna: venta.sucursalId.direccionId.comuna,
                    ciudad: venta.sucursalId.direccionId.ciudad,
                    region: venta.sucursalId.direccionId.region
                } : null
            },
            dependencia: venta.dependenciaId ? {
                _id: venta.dependenciaId._id,
                nombre: venta.dependenciaId.nombre,
                direccion: venta.dependenciaId.direccionId ? {
                    _id: venta.dependenciaId.direccionId._id,
                    nombre: venta.dependenciaId.direccionId.nombre,
                    direccion: venta.dependenciaId.direccionId.direccion,
                    comuna: venta.dependenciaId.direccionId.comuna,
                    ciudad: venta.dependenciaId.direccionId.ciudad,
                    region: venta.dependenciaId.direccionId.region
                } : null
            } : null,
            documentoTributario: venta.documentoTributarioId || null,
            comentariosCobro: venta.comentariosCobro?.map(comentario => ({
                fecha: comentario.fecha,
                comentario: comentario.comentario,
                usuario: {
                    _id: comentario.userId?._id,
                    nombre: comentario.userId?.name,
                    email: comentario.userId?.email
                },
                createdAt: comentario.createdAt,
                updatedAt: comentario.updatedAt
            })) || [],
            historialEstados: venta.historialEstados || [],
            entregasEnLocal: venta.entregasEnLocal || [],
            // Nuevos campos agregados
            detalles: detallesVenta?.map(detalle => ({
                _id: detalle._id,
                glosa: detalle.glosa,
                codigo: detalle.codigo,
                codigoCilindro: detalle.codigoCilindro,
                subcategoriaCatalogoId: detalle.subcategoriaCatalogoId ? {
                    _id: detalle.subcategoriaCatalogoId._id,
                    nombre: detalle.subcategoriaCatalogoId.nombre,
                    cantidad: detalle.subcategoriaCatalogoId.cantidad,
                    unidad: detalle.subcategoriaCatalogoId.unidad,
                    nombreGas: detalle.subcategoriaCatalogoId.nombreGas,
                    sinSifon: detalle.subcategoriaCatalogoId.sinSifon,
                    urlImagen: detalle.subcategoriaCatalogoId.urlImagen,
                    categoriaCatalogoId: detalle.subcategoriaCatalogoId.categoriaCatalogoId ? {
                        _id: detalle.subcategoriaCatalogoId.categoriaCatalogoId._id,
                        nombre: detalle.subcategoriaCatalogoId.categoriaCatalogoId.nombre,
                        descripcion: detalle.subcategoriaCatalogoId.categoriaCatalogoId.descripcion,
                        seguir: detalle.subcategoriaCatalogoId.categoriaCatalogoId.seguir,
                        urlImagen: detalle.subcategoriaCatalogoId.categoriaCatalogoId.urlImagen,
                        tipo: detalle.subcategoriaCatalogoId.categoriaCatalogoId.tipo,
                        gas: detalle.subcategoriaCatalogoId.categoriaCatalogoId.gas,
                        elemento: detalle.subcategoriaCatalogoId.categoriaCatalogoId.elemento,
                        esIndustrial: detalle.subcategoriaCatalogoId.categoriaCatalogoId.esIndustrial,
                        esMedicinal: detalle.subcategoriaCatalogoId.categoriaCatalogoId.esMedicinal
                    } : null
                } : null,
                itemCatalogoIds: detalle.itemCatalogoIds?.map(item => ({
                    _id: item._id,
                    codigo: item.codigo,
                    estado: item.estado,
                    ubicacion: item.ubicacion,
                    fechaCreacion: item.fechaCreacion,
                    fechaVencimiento: item.fechaVencimiento,
                    observaciones: item.observaciones
                })) || [],
                tipo: detalle.tipo,
                cantidad: detalle.cantidad,
                especifico: detalle.especifico,
                neto: detalle.neto,
                iva: detalle.iva,
                total: detalle.total
            })) || [],
            rutaDespacho: rutaDespacho ? {
                _id: rutaDespacho._id,
                vehiculo: rutaDespacho.vehiculoId ? {
                    _id: rutaDespacho.vehiculoId._id,
                    patente: rutaDespacho.vehiculoId.patente,
                    marca: rutaDespacho.vehiculoId.marca,
                    modelo: rutaDespacho.vehiculoId.modelo,
                    año: rutaDespacho.vehiculoId.año,
                    capacidad: rutaDespacho.vehiculoId.capacidad
                } : null,
                chofer: rutaDespacho.choferId ? {
                    _id: rutaDespacho.choferId._id,
                    nombre: rutaDespacho.choferId.name,
                    email: rutaDespacho.choferId.email
                } : null,
                dependencia: rutaDespacho.dependenciaId ? {
                    _id: rutaDespacho.dependenciaId._id,
                    nombre: rutaDespacho.dependenciaId.nombre,
                    direccion: rutaDespacho.dependenciaId.direccionId ? {
                        _id: rutaDespacho.dependenciaId.direccionId._id,
                        nombre: rutaDespacho.dependenciaId.direccionId.nombre,
                        direccionOriginal: rutaDespacho.dependenciaId.direccionId.direccionOriginal,
                        latitud: rutaDespacho.dependenciaId.direccionId.latitud,
                        longitud: rutaDespacho.dependenciaId.direccionId.longitud,
                        comuna: rutaDespacho.dependenciaId.direccionId.comuna,
                        ciudad: rutaDespacho.dependenciaId.direccionId.ciudad,
                        region: rutaDespacho.dependenciaId.direccionId.region,
                        codigoPostal: rutaDespacho.dependenciaId.direccionId.codigoPostal
                    } : null
                } : null,
                horaInicio: rutaDespacho.horaInicio,
                horaDestino: rutaDespacho.horaDestino,
                estado: rutaDespacho.estado,
                ruta: rutaDespacho.ruta?.map(parada => ({
                    direccionDestino: parada.direccionDestinoId ? {
                        _id: parada.direccionDestinoId._id,
                        nombre: parada.direccionDestinoId.nombre,
                        direccionOriginal: parada.direccionDestinoId.direccionOriginal,
                        latitud: parada.direccionDestinoId.latitud,
                        longitud: parada.direccionDestinoId.longitud,
                        comuna: parada.direccionDestinoId.comuna,
                        ciudad: parada.direccionDestinoId.ciudad,
                        region: parada.direccionDestinoId.region,
                        codigoPostal: parada.direccionDestinoId.codigoPostal,
                        categoria: parada.direccionDestinoId.categoria
                    } : null,
                    fechaArribo: parada.fechaArribo
                })) || [],
                historialEstado: rutaDespacho.historialEstado || [],
                historialCarga: rutaDespacho.historialCarga?.map(carga => ({
                    esCarga: carga.esCarga,
                    fecha: carga.fecha,
                    itemMovidoIds: carga.itemMovidoIds || []
                })) || [],
                cargaItemIds: rutaDespacho.cargaItemIds?.map(item => ({
                    _id: item._id,
                    codigo: item.codigo,
                    estado: item.estado,
                    ubicacion: item.ubicacion,
                    fechaCreacion: item.fechaCreacion,
                    fechaVencimiento: item.fechaVencimiento,
                    observaciones: item.observaciones,
                    subcategoriaCatalogoId: item.subcategoriaCatalogoId ? {
                        _id: item.subcategoriaCatalogoId._id,
                        nombre: item.subcategoriaCatalogoId.nombre,
                        cantidad: item.subcategoriaCatalogoId.cantidad,
                        unidad: item.subcategoriaCatalogoId.unidad,
                        nombreGas: item.subcategoriaCatalogoId.nombreGas,
                        sinSifon: item.subcategoriaCatalogoId.sinSifon,
                        urlImagen: item.subcategoriaCatalogoId.urlImagen,
                        categoriaCatalogoId: item.subcategoriaCatalogoId.categoriaCatalogoId ? {
                            _id: item.subcategoriaCatalogoId.categoriaCatalogoId._id,
                            nombre: item.subcategoriaCatalogoId.categoriaCatalogoId.nombre,
                            descripcion: item.subcategoriaCatalogoId.categoriaCatalogoId.descripcion,
                            seguir: item.subcategoriaCatalogoId.categoriaCatalogoId.seguir,
                            urlImagen: item.subcategoriaCatalogoId.categoriaCatalogoId.urlImagen,
                            tipo: item.subcategoriaCatalogoId.categoriaCatalogoId.tipo,
                            gas: item.subcategoriaCatalogoId.categoriaCatalogoId.gas,
                            elemento: item.subcategoriaCatalogoId.categoriaCatalogoId.elemento,
                            esIndustrial: item.subcategoriaCatalogoId.categoriaCatalogoId.esIndustrial,
                            esMedicinal: item.subcategoriaCatalogoId.categoriaCatalogoId.esMedicinal
                        } : null
                    } : null
                })) || [],
                ventaIds: rutaDespacho.ventaIds || [],
                createdAt: rutaDespacho.createdAt,
                updatedAt: rutaDespacho.updatedAt
            } : null
        };

        console.log("Returning venta details.");
        return NextResponse.json({ ok: true, venta: ventaDetails });

    } catch (error) {
        console.error("ERROR", error);
        return NextResponse.json({ ok: false, error: "Internal Server Error" }, { status: 500 });
    }
}