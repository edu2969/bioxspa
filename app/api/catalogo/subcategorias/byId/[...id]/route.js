import { NextResponse } from "next/server";
import { getSupabaseServerClient } from "@/lib/supabase";

export async function GET(request, props) {
    const params = await props.params;
    const { id } = params;
    if(!id) {
        console.log("[subcategorias.byId GET] Debe venir el código");
        return NextResponse.json({ ok: false, error: "Se esperaba id" }, { status: 400});
    }
    
    const supabase = await getSupabaseServerClient();
    const { data: subcategoria, error } = await supabase
        .from('subcategorias_catalogo')
        .select(`id, cantidad, unidad, sinSifon:sin_sifon, categoriaCatalogoId:categoria_catalogo_id`)
        .eq('id', id[0])
        .single();
    
    if(error) {
        console.log("[subcategorias.byId GET] Error al consultar", error);
        return NextResponse.json({ ok: false, error: "Error al consultar" }, { status: 500 });
    }

    if(!subcategoria) {
        console.log("[subcategorias.byId GET] Not found");
        return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });
    }

    return NextResponse.json({ ok: true, subcategoria });
}
